...
- **Telegram**: если токен/чат не заданы — события всё равно пишутся в outbox только при победе/поражении, но worker помечает такие события как `failed` с ретраями (поведение безопасно для продакакшена).
